...
def processData(fileName, featureIndexes={}, itemsLimit=None, useIt=[]):
    ...
    for processedCnt, item in getItems(fileName, itemsLimit):
        ...
        for word in getWords(item["title"]+" "+item["description"], stemmRequired = False, correctWordRequired = False):
            if word in featureIndexes and len(featureIndexes[word]) == 100:
                curr_vector = curr_vector + np.array(featureIndexes[word])
        
        features.append(curr_vector)
        targets.append(int(item["is_blocked"]))
        item_ids.append(int(item["itemid"]))
   
                    
        if len(features)%500 == 0:                 
            logging.debug(processMessage+": "+str(len(features))+" items done")
                
        
    return np.array(features), targets, item_ids
 

#Read data vectors
def loadVectors(fileName, featureIndexes={}, itemsLimit=None, useIt=[]):
    print ' Reading vectors from disk '
    features = {}
    import codecs
    with  codecs.open(os.path.join(dataFolder, fileName), 'r', 'utf-8') as fvector:
        try:    
            for line in fvector:
                tokens = line.split(' ')
                #print tokens
                features[tokens[0]] = map(lambda x: float(x), tokens[1 : 100])
                #print tokens[1 : len(tokens)]
...
def main():
    ...
    featureIndexes = loadVectors(os.path.join(dataFolder, "output.txt"), itemsLimit = iLimit)
